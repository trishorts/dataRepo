...
- **Pick up at:** Schema v0 is drafted (`schema/datarepo.yaml` core + `schema/study/aging.yaml` stub; see `design/SCHEMA_V0.md`). Next: map `aging/design/QUESTIONS.md` v0.3 (168 Qs, top 10 first) onto it in `design/SCHEMA_COVERAGE.md`, then post thread 004 to aging (study-layer columns, R7/R16 producer columns). Validate with LinkML (`pip install linkml`; `linkml-lint`, `linkml-validate -C Bundle`). D1–D7 are locked.
