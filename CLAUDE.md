...
- **Pick up at:** Schema v0 is drafted (`schema/datarepo.yaml` core + `schema/study/aging.yaml` stub; see `design/SCHEMA_V0.md`). The benchmark is mapped: `design/SCHEMA_COVERAGE.md` (168 questions: 70 ✓, 94 waiting on a producer, 2 with no home: J12, P2). Next: post thread 004 to aging from the coverage doc's last section (both `aging/design/threads/dataRepo/` and our mirror `design/threads/aging/`), then fill in the field descriptions and check the QPX column mapping. Validate with LinkML (`pip install linkml`; `linkml-lint`, `linkml-validate -C Bundle`). D1–D7 are locked.
