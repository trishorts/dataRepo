...
# dataRepo aging study layer (v0.4.0)
...
| `age_source` | [AgeSource](#agesource) | yes | Where `age_raw` came from: an SDRF cell, or the study owner's curation from a named source. |
| `age_source_reference` | `string` |  | For a curated age, the source it was read from (for example 'PMID 40544948' or 'PRIDE PXD056433 project record'). NULL for an SDRF age, whose source is the SDRF. |
| `normalizer_version` | `string` |  | Version of the software that turned `age_raw` into `age_years` (SdrfAge). Two rows normalized by different versions are not comparable and this is how a caller can tell. NULL when no normalizer ran, e.g. a curated age converted by hand; never a curation label (that is `age_source`). |
...
Where a sample's `age_raw` came from (aging 071, DATAREPO-59). Kept apart from `normalizer_version`, which names software: putting a curation label there would give one column two meanings, and a caller comparing normalizer versions would compare a label.

| Value | Meaning |
|---|---|
| `sdrf` | Read from the dataset's SDRF (deposited or drafted) and normalized by SdrfAge. `normalizer_version` is the SdrfAge version. |
| `curated` | Entered by the study owner from a named source such as the deposit's PRIDE record or its paper. `age_source_reference` says which, and a curated row whose source cannot be named should not be delivered. |
