...
One read-only DuckDB statement. Results carry `columns`, `rows`, `row_count`, `truncated` and the
limits in force. Prefer the acceptance views — `psms_1pct`, `peptidoforms_1pct`,
`protein_groups_1pct` — which apply the producing search engine's rule once so no caller restates
it.
...
Where the rows carry `bundle_id` or `dataset_id`, the bundle list is narrowed to what the answer
actually touched and says so. Where they do not, it is the catalog's whole bundle set and says
*that* — it never guesses a narrower claim than the rows support.
